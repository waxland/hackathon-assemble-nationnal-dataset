# TODO - Dashboard Streamlit (Miroir du Front "Minerve.fr")

Ce document dresse la feuille de route pour aligner l'outil d'exploration Data interne (`dashboard.py` sous Streamlit) avec la vision Produit officielle du frontend `Minerve.fr` (telle que définie dans le projet principal React/Next.js).
...
- [ ] **2. Route `/investissements` -> "Vue Transversale"** :
...
- [ ] **3. Routes `/investissements/[programmeCode]` -> "Rapports par Programme"** :
  ...
  - La vue doit agréger, pour UN programme donné :
    - [ ] Son budget précis (CP 2024, 2025, 2026).
    - [ ] Sa taxonomie / ses mots-clés associés.
    - [ ] L'écho parlementaire ciblé (ex: les débats liés au 424 uniquement).
    - [ ] Les entreprises / startups de la French Tech financées (avec leur code NAF).

## 📊 Dataviz & Intégration Data (SQLite)

- [ ] **Remplacement des requêtes "brutes"** :
...
---
*Ce TODO garantit que l'outil Streamlit n'avance pas à l'aveugle, mais reste le brouillon Data parfait pour le rendu final sur Minerve.fr.*
